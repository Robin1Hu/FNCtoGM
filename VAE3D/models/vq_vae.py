...
def window_reverse(windows, window_size, H, W, L):
    ...
    x = windows.view(B, H // window_size, W // window_size, L // window_size, window_size, window_size, window_size, -1)
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        H, W, L = self.input_resolution
        B, T, C = x.shape
        assert T == H * W * L, "input feature has wrong size"
        ...
        shifted_x = window_reverse(attn_windows, self.window_size, H, W, L)  # B H' W' C
    ...
    def flops(self):
        ...
        H, W, L = self.input_resolution
        # norm1
        flops += self.dim * H * W * L
        # W-MSA/SW-MSA
        nW = H * W * L / self.window_size / self.window_size / self.window_size
        flops += nW * self.attn.flops(self.window_size * self.window_size * self.window_size)
        # mlp
        flops += 2 * H * W * L * self.dim * self.dim * self.mlp_ratio
        # norm2
        flops += self.dim * H * W * L
        return flops
...
class PatchMerging(nn.Module):
    ...
    def forward(self, x):
        ...
        B, T, C = x.shape
        assert T == H * W * L, "input feature has wrong size"
        assert H % 2 == 0 and W % 2 == 0 and L % 2 == 0, f"x size ({H}*{W}*{L}) are not even."
    ...
    def flops(self):
        H, W, L = self.input_resolution
        flops = H * W * L * self.dim
        flops += (H // 2) * (W // 2) * (L // 2) * 8 * self.dim * 2 * self.dim
        return flops
...
class PatchExpand(nn.Module):
    def __init__(self, input_resolution, dim, dim_scale=2, norm_layer=nn.LayerNorm):
        ...
        self.input_resolution = input_resolution
        self.dim = dim
        self.expand = nn.Linear(dim, dim*4, bias=False) if dim_scale==2 else nn.Identity()
    ...
    def forward(self, x):
        ...
        x = x.view(B, H, W, L, C)
        #print(x.shape)
        x = rearrange(x, 'b h w l (p1 p2 p3 c)-> b (h p1) (w p2) (l p3) c', p1=2, p2=2, p3=2, c=C//8)
        x = x.view(B,-1,C//8)
        x = self.norm(x)
...
class FinalPatchExpand_X4(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.view(B, H, W, L, C)
        x = rearrange(x, 'b h w l (p1 p2 p3 c)-> b (h p1) (w p2) (l p3) c', p1=self.dim_scale, p2=self.dim_scale, p3=self.dim_scale, c=C//(self.dim_scale**3))
...
class PatchEmbed(nn.Module):
    ...
    def flops(self):
        ...
        flops = Ho * Wo * Lo * self.embed_dim * self.in_chans * (self.patch_size[0] * self.patch_size[1] * self.patch_size[2])
        if self.norm is not None:
...
class VQVAE(BaseVAE):
    ...
    def __init__(self, img_size=64, patch_size=4, in_chans=1, num_classes=1000,
        ...
        if self.final_upsample == "expand_first":
            ...
            self.up = FinalPatchExpand_X4(input_resolution=(img_size // patch_size, img_size // patch_size, img_size // patch_size),
    ...
    def decode(self, x, x_downsample):
    ...
    def up_x4(self, x):
        H, W, L = self.patches_resolution
        B, T, C = x.shape
        assert T == H * W * L, "input features has wrong size"

        if self.final_upsample == "expand_first":
            x = self.up(x)
            x = x.view(B, 4 * H, 4 * W, 4 * L, -1)
    ...
    def forward(self, x):
        x, x_downsample = self.encode(x)
        x = self.decode(x, x_downsample)
        print(x.shape)
        x = self.up_x4(x)
...
def main():
    ...
    # Create a dummy input tensor [b, 1, 64, 64, 64]
    dummy_input = torch.randn(4, 1, 64, 64, 64).to(device)  # Batch size of 4

    # Forward pass through the VQVAE
    output, original, vq_loss = vq_vae(dummy_input)

    # Print output shapes and vq_loss
    print("Output Shape:", output.shape)
    print("Original Input Shape:", original.shape)
    print("VQ Loss:", vq_loss.item())

    # Loss function calculation (optional, to see how the model calculates losses)
    losses = vq_vae.loss_function(output, original, vq_loss)
    print("Overall Loss:", losses['loss'].item())
    print("Reconstruction Loss:", losses['Reconstruction_Loss'].item())
    print("VQ Loss:", losses['VQ_Loss'].item())
...
if __name__ == "__main__":
    ...
    dummy_input = torch.randn(4, 1, 64, 64, 64)
